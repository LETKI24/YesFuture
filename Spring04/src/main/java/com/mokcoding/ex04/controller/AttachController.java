...
@Controller
@Log4j
public class AttachController { // 게시글과 첨부파일은 동기 방식으로 묶는게 낫다.
   ...
   // 첨부 파일 업로드 처리(POST)
   @PostMapping("/attach")
   public String attachPOST(AttachVO attachVO) { // VO에 들어가는거라고는 파일 하나 들어감
      log.info("attachPost()");
      log.info("attachDTO = " + attachVO);
      MultipartFile file = attachVO.getFile(); // 이 줄이 엄청난 힌트라고 함 (attachVO.getFile()특히 이 부분)
    ...
    @ResponseBody // 다운로드는 페이지를 불러오는게 아니고 리소스 클라한테 던져주는 형태라 ResponseBody가 필요하다 
